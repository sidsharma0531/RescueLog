...
// Gleaning capture (Glean Kentucky): trip-level produce logging. Volunteers
// photograph the recovered produce and the AI estimates the weight — no manual
// weighing. An optional scale-weight field exists as a fallback for the rare
// trip that does get weighed; when filled it overrides the AI total (the AI
// still provides the category breakdown). Reuses the pop-up flow's GPS
// location + photo + upload pipeline. Kept deliberately simple: location,
// photos, submit.
export default function GleaningCaptureScreen({ navigation }) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // Create the gleaning log. The AI estimates the weight from photos; the
      // scale weight is an optional override.
      const scale = Number(scaleWeight);
      const popup = await api.createPopup({
        ...
        scale_weight_lbs:
          scaleWeight && Number.isFinite(scale) && scale > 0 ? scale : null,
      ...
      setUploadProgress(null); // uploads done — AI now analyzes the produce
        ...
        >
          ...
          <View style={styles.card}>
            ...
            <Text style={styles.hint}>
              The AI estimates the weight and produce breakdown from your
              photos. No weighing needed.
            </Text>
          </View>

          <View style={styles.card}>
            <Text style={styles.cardTitle}>Scale weight (optional)</Text>
            <TextInput
              value={scaleWeight}
              onChangeText={(t) =>
            ...
            />
            <Text style={styles.hint}>
              Only if you happened to weigh this load. It overrides the AI
              total.
            </Text>
          </View>
